...
export const createV1Router = (deps: V1RouterDeps): Router => {
  ...
  const roomLoader = async (externalRoomId: string, tenantId: string, botId?: string): Promise<BotRoom | null> => {
    if (botId == null) {
      return null
    }
    return await deps.roomService.findByExternalRoomId(tenantId, botId, externalRoomId)
  }
